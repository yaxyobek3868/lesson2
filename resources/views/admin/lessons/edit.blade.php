...
<form action="{{ route('lessons.update', $lesson->id) }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <select name="user_id" id="user_id" class="form-select" required>
            ...
            @foreach($users as $user)
                <option value="{{ $user->id }}" {{ (old('user_id', $lesson->user_id) == $user->id) ? 'selected' : '' }}>
                    {{ $user->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="mb-3">
        ...
        <textarea name="text" id="text" rows="4" class="form-control" required>{{ old('text', $lesson->text) }}</textarea>
    </div>

    <div class="mb-3">
        <label for="kuni" class="form-label">Sana</label>
        <input type="date" name="kuni" id="kuni" class="form-control" value="{{ old('kuni', $lesson->kuni) }}" required>
    </div>

    <div class="mb-3">
        <label for="soati" class="form-label">Soat</label>
        <input type="time" name="soati" id="soati" class="form-control" value="{{ old('soati', $lesson->soati) }}" required>
    </div>
